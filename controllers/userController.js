...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  // Allow only @nsec.ac.in email addresses
  if (!email.endsWith("@nsec.ac.in")) {
    return next(new ErrorHandler("Only @nsec.ac.in emails are allowed for registration!", 400));
  }
...
export const toggleUserActiveStatus = catchAsyncErrors(async (req, res, next) => {
  const { userId } = req.params;
